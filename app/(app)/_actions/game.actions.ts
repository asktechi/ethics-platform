...
import { defaultGameFilter, defaultGameSettings, type GameMode, type WizardState } from "@/lib/games/types";
...
const wizardSchema = z.object({
  ...
  mode: z.enum(["jeopardy", "rapid_fire", "case_study", "team_battle", "adaptive", "boss_battle"]),
  settings: z.object({
...
export async function countFilterAction(classId: string, filter: unknown) {
  try {
    const parsed = wizardSchema.shape.filter.parse(filter);
    const count = await countFilterMatches(classId, parsed);
